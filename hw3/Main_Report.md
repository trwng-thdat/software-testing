...
> **Cập nhật:** Cột `Result`/`Notes` bên dưới đã được điền từ kết quả **thực thi tự động thật** bằng bộ Selenium (`selenium/run_checklist.py`, chạy trên Chrome, single browser), không còn là suy luận tĩnh từ code. Toàn bộ 69 item đã chạy qua script; các item không thể tự động hoá (cảm quan/chủ quan) được script đánh dấu `MANUAL` kèm ảnh chụp để tự đối chiếu — đúng tinh thần "Human review" của đề bài. Báo cáo thực thi đầy đủ (HTML + Markdown): `selenium/report/index.html`, `selenium/report/report.md`. Ảnh chụp toàn bộ 69 item (kể cả PASS): `selenium/screenshots/<ID>.png`. **Ảnh của riêng các item FAIL/MANUAL** (43 ảnh) đã được sao chép vào [`hw3/screenshot/`](screenshot/) và link trực tiếp trong từng dòng bảng bên dưới.
>
...
> Nguồn dữ liệu: `selenium/results/results.csv` + `selenium/report/report.md` (chạy lần cuối trên Chrome headless, có bật `--include-lockout`), kết hợp xem trực tiếp toàn bộ ảnh chụp tại [`hw3/screenshot/`](screenshot/) để đưa ra kết luận PASS/FAIL cuối cùng cho các item cảm quan/chủ quan. Mỗi dòng FAIL đều có giải thích cụ thể dựa trên nội dung ảnh (và đối chiếu code khi cần) trong cột Notes — không còn item nào ở trạng thái `TODO`/`MANUAL` treo.
>
...
>
> **Đã tạo GitHub Issue cho toàn bộ 30 bug** trên repo [`DuyITLOR/group05_eshop`](https://github.com/DuyITLOR/group05_eshop/issues) — đánh số liên tục từ [#125](https://github.com/DuyITLOR/group05_eshop/issues/125) (BUG-GUI-01) đến [#154](https://github.com/DuyITLOR/group05_eshop/issues/154) (BUG-GUI-30), link trực tiếp ở cột GitHub Issue trong bảng trên. Ảnh chụp minh chứng tương ứng có sẵn tại [`hw3/screenshot/`](screenshot/) (cột Screenshot).

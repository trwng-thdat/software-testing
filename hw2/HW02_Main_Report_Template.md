...
| -------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ---------------------------------------------------------------- | -------------------------------- |
| BUG-A-01 | TC-PROFILE-012                                                                                 | Privilege escalation: user thường tự gán `role=admin` qua `PUT /api/users/me` (server.js:124)                                                                                  | Critical | [#58](https://github.com/DuyITLOR/group05_eshop/issues/58)   | ![BUG-A-01](images/pool_a/1.png) |
| BUG-A-02 | TC-PROFILE-004                                                                                 | Stored XSS: `name` chứa HTML/script lưu raw vào DB; navbar Web render qua `dangerouslySetInnerHTML` (App.jsx:27) → script thực thi                                             | High     | [#59](https://github.com/DuyITLOR/group05_eshop/issues/59)   | ![BUG-A-02](images/pool_a/2.png) |
| BUG-A-03 | TC-PROFILE-002, TC-PROFILE-003, TC-PROFILE-005, TC-PROFILE-007, TC-PROFILE-008, TC-PROFILE-010 | Thiếu validation server-side: `name`, `phone`, `shipping_address` không được validate tại server — bypass qua Postman bỏ qua toàn bộ ràng buộc client                          | High     | [#60](https://github.com/DuyITLOR/group05_eshop/issues/60)   | ![BUG-A-03](images/pool_a/3.png) |
| BUG-A-04 | TC-PROFILE-011                                                                                 | Stored XSS: `shipping_address` chứa XSS payload lưu raw; potential render ở các trang hiển thị địa chỉ                                                                         | Medium   | [#61](https://github.com/DuyITLOR/group05_eshop/issues/61)   | ![BUG-A-04](images/pool_a/4.png) |
| BUG-A-05 | TC-PROFILE-006                                                                                 | Design bug trong regex phone: `^[1-9][0-9]{8,9}$` từ chối số VN hợp lệ bắt đầu bằng 0 (vd `0912345678`) — số VN thật đều bắt đầu bằng 0                                        | Medium   | [#62](https://github.com/DuyITLOR/group05_eshop/issues/62) | ![BUG-A-05](images/pool_a/5.png) |
| BUG-A-06 | _(Mobile test)_                                                                                | Mobile field name mismatch: `frontend-mobile/App.js:302` gửi `shippingAddress` (camelCase) nhưng server đọc `shipping_address` → address không bao giờ lưu được qua mobile app | Medium   | [#63](https://github.com/DuyITLOR/group05_eshop/issues/63) | ![BUG-A-06](images/pool_a/6.png) |
| BUG-A-07 | TC-PROFILE-015                                                                                 | Sensitive Data Exposure: `GET /api/users/me` dùng `SELECT *` (server.js:113) trả cả `password` (lưu plaintext) và `reset_token` về client                                      | Critical | [#64](https://github.com/DuyITLOR/group05_eshop/issues/64) | ![BUG-A-07](images/pool_a/7.png) |
| BUG-A-08 | TC-PROFILE-016                                                                                 | Data loss: `PUT /api/users/me` luôn `SET name,shipping_address,phone` (server.js:121) → cập nhật thiếu field sẽ ghi NULL đè dữ liệu cũ (không hỗ trợ partial update)           | High     | [#65](https://github.com/DuyITLOR/group05_eshop/issues/65) | ![BUG-A-08](images/pool_a/8.png) |
...
| Bug ID   | Found by TC                                                                                          | Tiêu đề                                                                                                                                                                                                                                      | Severity | GitHub Issue                                                     | Screenshot                       |
| -------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------- | -------------------------------- |
| BUG-B-01 | TC-CHECKOUT-102                                                                                      | Off-by-one: `apply-coupon` dùng `total > min_order_amount` thay vì `>=` → đơn đúng ngưỡng tối thiểu bị từ chối (server.js:379)                                                                                                               | High     | [#66](https://github.com/DuyITLOR/group05_eshop/issues/66) | ![BUG-B-01](images/pool_b/1.png) |
| BUG-B-02 | TC-CHECKOUT-009, TC-CHECKOUT-103                                                                     | Sai công thức percent discount: `total*(1-discount_value)` với `discount_value=10` (số nguyên) → discount âm, final > total (server.js:399)                                                                                                  | Critical | [#67](https://github.com/DuyITLOR/group05_eshop/issues/67) | ![BUG-B-02](images/pool_b/2.png) |
| BUG-B-03 | TC-CHECKOUT-002, TC-CHECKOUT-003, TC-CHECKOUT-004, TC-CHECKOUT-005, TC-CHECKOUT-006, TC-CHECKOUT-007 | Thiếu validation server-side cho checkout: `total_amount` (0, âm, chuỗi) và `shipping_address` (rỗng, null) đều được chấp nhận (server.js:297-309)                                                                                           | High     | [#68](https://github.com/DuyITLOR/group05_eshop/issues/68) | ![BUG-B-03](images/pool_b/3.png) |
| BUG-B-04 | TC-CHECKOUT-015                                                                                      | Bypass `max_uses_per_user`: `apply-coupon` không có `authenticateToken`; khi bỏ field `user_id`, nhánh `if (user_id)` (server.js:386) = false → bỏ qua toàn bộ kiểm tra số lần dùng → mã giới hạn lượt vẫn áp được vô hạn bởi caller ẩn danh | Critical | [#69](https://github.com/DuyITLOR/group05_eshop/issues/69) | ![BUG-B-04](images/pool_b/4.png) |
...
| Bug ID   | Found by TC                            | Tiêu đề                                                                                                                                                                                                         | Severity | GitHub Issue | Screenshot |
| -------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------ | ---------- |
| BUG-C-01 | TC-ADMIN_ORDER-002, TC-ADMIN_ORDER-013 | Authorization bypass: `/api/admin/*` không kiểm tra `role=admin` → user thường đọc/sửa được đơn (server.js:510/525)                                                                                             | Critical | [#70](https://github.com/DuyITLOR/group05_eshop/issues/70)   | ![BUG-C-01](images/pool_c/1.png)    |
| BUG-C-02 | TC-ADMIN_ORDER-010                     | State machine bug: `canceled → delivered` được cho phép (server.js:550-551) — đơn đã hủy lại thành đã giao                                                                                                      | High     | [#71](https://github.com/DuyITLOR/group05_eshop/issues/71)   | ![BUG-C-02](images/pool_c/2.png)    |
| BUG-C-03 | TC-ADMIN_ORDER-016, TC-ADMIN_ORDER-017 | Thiếu input validation cho `status`: body thiếu field hoặc `status=""` không bị chặn (server.js:526) → lọt vào kiểm transition, trả thông điệp sai `"...to undefined"`/`"...to "` thay vì 400 "status bắt buộc" | Medium   | [#72](https://github.com/DuyITLOR/group05_eshop/issues/72)   | ![BUG-C-03](images/pool_c/3.png)    |
...
| Bug ID   | Found by TC                                              | Tiêu đề                                                                                                                                                                                                     | Severity | GitHub Issue | Screenshot |
| -------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------ | ---------- |
| BUG-D-01 | TC-MOB_REG-002..008, TC-MOB_REG-009..011, TC-MOB_REG-101 | Thiếu server-side validation: `POST /api/register` (server.js:20-30) không validate name/email/password — gửi rỗng, sai định dạng, thiếu field đều được INSERT thẳng vào DB                                 | High     | [#73](https://github.com/DuyITLOR/group05_eshop/issues/73)   | ![BUG-D-01](images/pool_d/1.png)    |
| BUG-D-02 | TC-MOB_REG-004                                           | Duplicate email: `database.js` không có UNIQUE constraint trên cột `email` + server không check trùng → đăng ký nhiều tài khoản cùng email thành công                                                       | High     | [#74](https://github.com/DuyITLOR/group05_eshop/issues/74)   | ![BUG-D-02](images/pool_d/2.png)    |
| BUG-D-03 | TC-MOB_REG-012                                           | Plaintext password storage: `server.js:23` INSERT password trực tiếp không qua hashing (bcrypt/argon2) → lộ mật khẩu nếu DB bị truy cập                                                                     | Critical | [#75](https://github.com/DuyITLOR/group05_eshop/issues/75)   | ![BUG-D-03](images/pool_d/3.png)    |
| BUG-D-04 | TC-MOB_REG-013                                           | Regex client không khớp whitelist SRS: `App.js:212` dùng `[^A-Za-z\d]` chấp nhận MỌI ký tự đặc biệt (`#`, `^`, dấu cách...), trong khi SRS FR-01 chỉ cho phép `@ $ ! % * ? &` → validation rộng hơn yêu cầu | Medium   | [#76](https://github.com/DuyITLOR/group05_eshop/issues/76)   | ![BUG-D-04](images/pool_d/4.png)    |
| BUG-D-05 | TC-MOB_REG-014                                           | Thiếu trường "Xác nhận mật khẩu": form Register mobile (`App.js`) không có confirm password — vi phạm SRS FR-01 (README.md:35) bắt buộc có trường xác nhận và từ chối khi không khớp                        | Medium   | [#77](https://github.com/DuyITLOR/group05_eshop/issues/77)   | ![BUG-D-05](images/pool_d/5.png)    |
...
| -------- | ------- | ---------------------------------- | ----------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------- | ---------------------------------------------------------------- |
| BUG-A-01 | A       | TC-PROFILE-012                     | Privilege escalation: user thường tự gán `role=admin` qua `PUT /api/users/me`                   | Critical | Confirmed (executed)                                  | [#58](https://github.com/DuyITLOR/group05_eshop/issues/58)   |
| BUG-A-02 | A       | TC-PROFILE-004                     | Stored XSS qua `name` + `dangerouslySetInnerHTML` (App.jsx:27)                                  | High     | Confirmed (executed)                                  | [#59](https://github.com/DuyITLOR/group05_eshop/issues/59)   |
| BUG-A-03 | A       | TC-PROFILE-002/003/005/007/008/010 | Thiếu validation server-side cho `name`/`phone`/`shipping_address`                              | High     | Confirmed (executed)                                  | [#60](https://github.com/DuyITLOR/group05_eshop/issues/60)   |
| BUG-A-04 | A       | TC-PROFILE-011                     | Stored XSS qua `shipping_address`                                                               | Medium   | Confirmed (executed)                                  | [#61](https://github.com/DuyITLOR/group05_eshop/issues/61)   |
| BUG-A-05 | A       | TC-PROFILE-006                     | Design bug regex phone: từ chối số VN hợp lệ bắt đầu bằng `0`                                   | Medium   | Confirmed (executed)                                  | [#62](https://github.com/DuyITLOR/group05_eshop/issues/62) |
| BUG-A-06 | A       | _(Mobile — execute ở Feature D)_   | Mobile field mismatch: App.js gửi `shippingAddress` (camelCase) ≠ server đọc `shipping_address` | Medium   | Open — xác nhận qua đọc code; cần execute trên mobile | [#63](https://github.com/DuyITLOR/group05_eshop/issues/63) |
| BUG-A-07 | A       | TC-PROFILE-015                     | Sensitive Data Exposure: `GET /api/users/me` `SELECT *` trả cả `password`/`reset_token`         | Critical | Confirmed (executed)                                  | [#64](https://github.com/DuyITLOR/group05_eshop/issues/64) |
| BUG-A-08 | A       | TC-PROFILE-016                     | Data loss: partial update ghi NULL đè dữ liệu cũ                                                | High     | Confirmed (executed)                                  | [#65](https://github.com/DuyITLOR/group05_eshop/issues/65) |
| BUG-B-01 | B       | TC-CHECKOUT-102                    | Off-by-one: apply-coupon dùng `>` thay vì `>=` (server.js:379)                                  | High     | Confirmed (executed)                                  | [#66](https://github.com/DuyITLOR/group05_eshop/issues/66) |
| BUG-B-02 | B       | TC-CHECKOUT-009, TC-CHECKOUT-103   | Sai công thức percent discount → discount âm, final > total (server.js:399)                     | Critical | Confirmed (executed)                                  | [#67](https://github.com/DuyITLOR/group05_eshop/issues/67) |
| BUG-B-03 | B       | TC-CHECKOUT-002…007                | Thiếu validation server-side cho checkout (`total_amount`, `shipping_address`)                  | High     | Confirmed (executed)                                  | [#68](https://github.com/DuyITLOR/group05_eshop/issues/68) |
| BUG-B-04 | B       | TC-CHECKOUT-015                    | Bypass `max_uses_per_user`: bỏ field `user_id` → bỏ qua kiểm tra số lần dùng                    | Critical | Confirmed (executed)                                  | [#69](https://github.com/DuyITLOR/group05_eshop/issues/69) |
| BUG-C-01 | C       | TC-ADMIN_ORDER-002, -013           | Authorization bypass: `/api/admin/*` không kiểm tra `role=admin`                                | Critical | Confirmed (executed)                                  | [#70](https://github.com/DuyITLOR/group05_eshop/issues/70)                                                       |
| BUG-C-02 | C       | TC-ADMIN_ORDER-010                 | State machine bug: cho phép `canceled → delivered` (server.js:550-551)                          | High     | Confirmed (executed)                                  | [#71](https://github.com/DuyITLOR/group05_eshop/issues/71)                                                       |
| BUG-C-03 | C       | TC-ADMIN_ORDER-016, -017           | Thiếu input validation cho `status` (thiếu field / rỗng) → thông điệp lỗi sai                   | Medium   | Confirmed (executed)                                  | [#72](https://github.com/DuyITLOR/group05_eshop/issues/72)                                                       |
| BUG-D-01 | D       | TC-MOB_REG-002…011, -101           | Thiếu server-side validation `POST /api/register` (name/email/password)                         | High     | Confirmed (executed)                                  | [#73](https://github.com/DuyITLOR/group05_eshop/issues/73)                                                       |
| BUG-D-02 | D       | TC-MOB_REG-004                     | Duplicate email: thiếu UNIQUE constraint + không check trùng                                    | High     | Confirmed (executed, count=2)                         | [#74](https://github.com/DuyITLOR/group05_eshop/issues/74)                                                       |
| BUG-D-03 | D       | TC-MOB_REG-012                     | Plaintext password storage (server.js:23)                                                       | Critical | Confirmed (executed, đọc DB)                          | [#75](https://github.com/DuyITLOR/group05_eshop/issues/75)                                                       |
| BUG-D-04 | D       | TC-MOB_REG-013                     | Regex client `[^A-Za-z\d]` rộng hơn whitelist SRS `{@$!%*?&}`                                   | Medium   | Confirmed (executed)                                  | [#76](https://github.com/DuyITLOR/group05_eshop/issues/76)                                                       |
| BUG-D-05 | D       | TC-MOB_REG-014                     | Thiếu trường "Xác nhận mật khẩu" trên mobile — vi phạm FR-01                                    | Medium   | Confirmed (code review)                               | [#77](https://github.com/DuyITLOR/group05_eshop/issues/77)                                                       |
...
**(1) Thiên về input, bỏ quên output.** AI mặc định "domain testing = validate đầu vào" nên hầu như không assert nội dung response. Hệ quả là bỏ sót rò rỉ dữ liệu nhạy cảm (BUG-A-07: `GET /me` trả `password` plaintext), nội dung JOIN của admin (C gap #6), và việc response đăng ký có lộ field nhạy cảm không (D gap #3).

**(2) Gộp "field thiếu" với "field rỗng".** AI coi `undefined` (vắng field) và `""` (rỗng) là một lớp, trong khi với `const {x}=req.body` + SQLite chúng cho hành vi khác (NULL vs ""). Lặp ở A gap #2, C gap #3, D gap #1.

**(3) Oracle problem — lấy code làm chân lý.** AI chỉ bắt được "code làm sai cái nó có" (BUG-C-02), không thấy "code thiếu cái SRS đòi": thiếu trường xác nhận mật khẩu (BUG-D-05) và regex rộng hơn whitelist SRS (BUG-D-04) chỉ lộ khi con người đối chiếu README/FR-01 — tài liệu mà skill không nạp cho AI.

**(4) Ảo giác độ phủ.** AI gán khống EC↔TC cho bảng truy vết trông kín (C gap #1, #2), dù TC không thực sự kích hoạt lớp đó.

**(5) Tuân thủ quy tắc máy móc.** "Không có biên định nghĩa" bị hiểu thành "không cần test" (A gap #6).

**Nguyên tắc rút ra:** (a) human review là bắt buộc — phải tự execute và đọc DB để xác nhận, không tin output AI; (b) phải cấp spec nghiệp vụ độc lập làm oracle, không để AI tự lấy code làm chuẩn; (c) phải dò chéo bảng coverage vì AI hay khai khống; (d) AI là công cụ tăng tốc bản nháp, trách nhiệm đúng/sai thuộc về người kiểm thử.
...
| :-- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------- | :---------- | :-------------------- | :-------------- |
| 1   | Tool: Claude Code — claude-opus-4-8[1m] · Time: 25/06/2026 · Prompt: "Giờ tôi muốn viết skill cho bạn thì làm như nào. tạo cho tôi 1 chỗ để chứa skill"                                                                                      | Verbatim — `audit.md` › Artifact #1  | `[ ]`       |                       |                 |
| 2   | Tool: Claude Code — claude-opus-4-8[1m] · Time: 25/06/2026 · Prompt: "đọc file @software-testing/hw2/2026.HW02.Domain Testing_En.pdf và cho tôi biết homework này gồm nội dung về cái gì và tôi cần phải làm gì. gen ra 1 file md trong folder hw2 luôn" | Verbatim — `audit.md` › Artifact #2  | `[ ]`       |                       |                 |
| 3   | Tool: Claude Code — claude-sonnet-4-6 · Time: 25/06/2026 · Prompt: "[Bảng phân công feature của nhóm — Pool A/B/C/D với người phụ trách từng FR] Theo bạn thì tôi nên chọn cái nào ở mỗi pool"                                                | Verbatim — `audit.md` › Artifact #3  | `[ ]`       |                       |                 |
| 4   | Tool: Claude Code — claude-sonnet-4-6 · Time: 25/06/2026 · Prompt: "Bây giờ tôi muốn bạn viết cho tôi 1 skill là tự động viết audit log template mẫu ở @software-testing/templates/[AI-02] - FIT@HCMUS - AI Audit Report_En.docx.md . mỗi lần gen thì gen ra 1 bản audit.md ở ngoài cùng cùng cấp với folder software testing. và đặc biệt phải lưu ý rằng output phải là raw. không được summary lại" | Verbatim — `audit.md` › Artifact #4  | `[ ]`       |                       |                 |
| 5   | Tool: Claude Code — claude-sonnet-4-6 · Time: 25/06/2026 · Prompt: "Nội dung file bằng tiếng anh nhưng note lại khi gen file audit thì ghi tiếng việt. chỉ có nội dung prompt và output thì tùy và lúc đó như nào."                            | Verbatim — `audit.md` › Artifact #5  | `[ ]`       |                       |                 |
| 6   | Tool: Claude Code — claude-sonnet-4-6 · Time: 25/06/2026 · Prompt: "Bạn có kêu phải quay video. video đây là quay cái gì"                                                                                                                    | Verbatim — `audit.md` › Artifact #6  | `[ ]`       |                       |                 |
| 7   | Tool: Claude Code — claude-sonnet-4-6 · Time: 25/06/2026 · Prompt: "chỉ cần 1 video thôi hay phải quay hết cả 4 pool"                                                                                                                        | Verbatim — `audit.md` › Artifact #7  | `[ ]`       |                       |                 |
| 8   | Tool: Claude Code — claude-sonnet-4-6 · Time: 25/06/2026 · Prompt: "Vậy bài này đơn giản là với mỗi pool thì cần dựa vào các kĩ thuật cung cấp gen ra các test case và mình phải test đúng không"                                            | Verbatim — `audit.md` › Artifact #8  | `[ ]`       |                       |                 |
| 9   | Tool: Claude Code — claude-opus-4-8[1m] · Time: 25/06/2026 · Prompt: "Viết cho tôi skill gen test case theo kĩ thuật domain testing"                                                                                                         | Verbatim — `audit.md` › Artifact #9  | `[ ]`       |                       |                 |
| 10  | Tool: Claude Code — claude-opus-4-8[1m] · Time: 25/06/2026 · Prompt: "Tiếp tục với Boundary Value Analysis"                                                                                                                                  | Verbatim — `audit.md` › Artifact #10 | `[ ]`       |                       |                 |
| 11  | Tool: Claude Code — claude-opus-4-8[1m] · Time: 25/06/2026 · Prompt: "Dựa vào @software-testing/hw2/2026.HW02.Domain Testing_En.pdf gen cho tôi ra 1 file template bằng markdown để tôi chỉ việc fill in dữ liệu vào."                         | Verbatim — `audit.md` › Artifact #11 | `[ ]`       |                       |                 |
| 12  | Tool: Claude Code — claude-opus-4-8[1m] · Time: 25/06/2026 · Prompt: "dùng skill để gen ra file chứa audit cho tôi đi. nội dung là prompt bên trên"                                                                                          | Verbatim — `audit.md` › Artifact #12 | `[ ]`       |                       |                 |
| 13  | Tool: Claude Code — claude-sonnet-4-6 · Time: 26/06/2026 · Prompt: "tạo cho tôi các test case cho feature A FR-04: Personal profile management. Số lượng test case tùy vào bạn, Hãy gen ra tới khi bạn cảm thấy đủ bao quát toàn bộ và không còn lỗi. và ghi lại các test case đó vào file @software-testing/hw2/HW02_Main_Report_Template.md theo format trong file." | Verbatim — `audit.md` › Artifact #13 | `[ ]`       |                       |                 |
| 14  | Tool: Claude Code — claude-sonnet-4-6 · Time: 26/06/2026 · Prompt: "trong file hw2 thì chỗ nào kêu quay video. và quay cái gì"                                                                                                               | Verbatim — `audit.md` › Artifact #14 | `[ ]`       |                       |                 |
| 15  | Tool: Claude Code — claude-sonnet-4-6 · Time: 26/06/2026 · Prompt: "Sử dụng skill boundary-value-analysis để tìm ra các test case cho feature A Personal profile management. gen xong thì ghi vào file software-testing/hw2/HW02_Main_Report_Template.md" | Verbatim — `audit.md` › Artifact #15 | `[ ]`       |                       |                 |
| 16  | Tool: Claude Code — claude-sonnet-4-6 · Time: 27/06/2026 · Prompt: "Dựa vào các testcases bạn đã liệt kê trong feature a ở software-testing/hw2/HW02_Main_Report_Template.md bạn hãy tạo 1 file .rest để tôi test"                            | Verbatim — `audit.md` › Artifact #16 | `[ ]`       |                       |                 |
| 17  | Tool: Claude Code — claude-sonnet-4-6 · Time: 27/06/2026 · Prompt: "Kiểm tra xem file featura a đã làm ổn chưa. chỗ nào ghi comment hay note giải thích để tôi tiện trong việc làm thì xóa đi. bạn cảm thấy note hoặc comment nào nên xóa thì xóa vì đây sẽ là bài nộp cho giáo viên" | Verbatim — `audit.md` › Artifact #17 | `[ ]`       |                       |                 |
| 18  | Tool: Claude Code — claude-sonnet-4-6 · Time: 27/06/2026 · Prompt: "tại sao với shipping_address thì không có bva"                                                                                                                           | Verbatim — `audit.md` › Artifact #18 | `[ ]`       |                       |                 |
| 19  | Tool: Claude Code — claude-sonnet-4-6 · Time: 27/06/2026 · Prompt: "Gen cho tôi các nội dung như title và description để tôi tạo issue [screenshot GitHub Create Issue form với title BUG-A-01]"                                              | Verbatim — `audit.md` › Artifact #19 | `[ ]`       |                       |                 |
| 20  | Tool: Claude Code — claude-sonnet-4-6 · Time: 27/06/2026 · Prompt: "bug-a-1…bug-a-8: [8 link GitHub Issues #6–#13]. Trên là toàn bộ link của từng bug. tôi cũng đã dán ảnh trên folder software-testing/hw2/images/pool_a/ file ảnh có name là 1 tương ứng với bug 1. fill in dữ liệu dô" | Verbatim — `audit.md` › Artifact #20 | `[ ]`       |                       |                 |
| 21  | Tool: Claude Code — claude-opus-4-8 · Time: 27/06/2026 · Prompt: "Sử dụng skill domain-testing và tạo ra các testcases cho feature B-CHECKOUT. ghi nội dung các testcase đó vào file software-testing/hw2/HW02_Main_Report_Template.md. nhớ bám theo format của file. có thể nhìn lên feature A để làm ví dụ." | Verbatim — `audit.md` › Artifact #21 | `[ ]`       |                       |                 |
| 22  | Tool: Claude Code — claude-sonnet-4-6 · Time: 27/06/2026 · Prompt: "Sử dụng skill domain-testing và boundary-value-analysis tạo ra các testcases cho feature C-Order management. ghi nội dung các testcase đó vào file @software-testing/hw2/HW02_Main_Report_Template.md . nhớ bám theo format của file. có thể nhìn lên feature A để làm ví dụ." | Verbatim — `audit.md` › Artifact #22 | `[ ]`       |                       |                 |
| 23  | Tool: Claude Code — claude-sonnet-4-6 · Time: 27/06/2026 · Prompt: "Sử dụng skill domain-testing và tạo ra các testcases cho feature D-Mobile – Registration. ghi nội dung các testcase đó vào file @software-testing/hw2/HW02_Main_Report_Template.md . nhớ bám theo format của file. có thể nhìn lên feature A để làm ví dụ." | Verbatim — `audit.md` › Artifact #23 | `[ ]`       |                       |                 |
| 24  | Tool: Claude Code — claude-sonnet-4-6 · Time: 27/06/2026 · Prompt: "Sử dụng skill boundary-value-analysis và tạo ra các testcases cho feature D-Mobile – Registration. ghi nội dung các testcase đó vào file @software-testing/hw2/HW02_Main_Report_Template.md . nhớ bám theo format của file. có thể nhìn lên feature A để làm ví dụ." | Verbatim — `audit.md` › Artifact #24 | `[ ]`       |                       |                 |
| 25  | Tool: Claude Code — claude-opus-4-8[1m] · Time: 28/06/2026 · Prompt: "Đọc file @software-testing/hw2/HW02_Main_Report_Template.md và đánh giá xem so với yêu cầu của đề bài @software-testing/hw2/2026.HW02.Domain Testing_En.pdf thì còn thiếu yêu cầu gì không. và có chỗ nào cần chỉnh sửa không và đề xuất lại cho tôi để tôi tham khảo" | Verbatim — `audit.md` › Artifact #25 | `[ ]`       |                       |                 |

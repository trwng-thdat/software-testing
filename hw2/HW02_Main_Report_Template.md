...
| GitHub repo (nhóm)  | https://github.com/trwng-thdat/software-testing        |
| GitHub Issues       | https://github.com/trwng-thdat/software-testing/issues |
...
| Bug ID   | Found by TC                                                                                          | Tiêu đề                                                                                                                                                                                                                                      | Severity | GitHub Issue                                                     | Screenshot                       |
| -------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------- | -------------------------------- |
| BUG-B-01 | TC-CHECKOUT-102                                                                                      | Off-by-one: `apply-coupon` dùng `total > min_order_amount` thay vì `>=` → đơn đúng ngưỡng tối thiểu bị từ chối (server.js:379)                                                                                                               | High     | [#14](https://github.com/trwng-thdat/software-testing/issues/14) | ![BUG-B-01](images/pool_b/1.png) |
| BUG-B-02 | TC-CHECKOUT-009, TC-CHECKOUT-103                                                                     | Sai công thức percent discount: `total*(1-discount_value)` với `discount_value=10` (số nguyên) → discount âm, final > total (server.js:399)                                                                                                  | Critical | [#15](https://github.com/trwng-thdat/software-testing/issues/15) | ![BUG-B-02](images/pool_b/2.png) |
| BUG-B-03 | TC-CHECKOUT-002, TC-CHECKOUT-003, TC-CHECKOUT-004, TC-CHECKOUT-005, TC-CHECKOUT-006, TC-CHECKOUT-007 | Thiếu validation server-side cho checkout: `total_amount` (0, âm, chuỗi) và `shipping_address` (rỗng, null) đều được chấp nhận (server.js:297-309)                                                                                           | High     | [#16](https://github.com/trwng-thdat/software-testing/issues/16) | ![BUG-B-03](images/pool_b/3.png) |
| BUG-B-04 | TC-CHECKOUT-015                                                                                      | Bypass `max_uses_per_user`: `apply-coupon` không có `authenticateToken`; khi bỏ field `user_id`, nhánh `if (user_id)` (server.js:386) = false → bỏ qua toàn bộ kiểm tra số lần dùng → mã giới hạn lượt vẫn áp được vô hạn bởi caller ẩn danh | Critical | [#17](https://github.com/trwng-thdat/software-testing/issues/17) | ![BUG-B-04](images/pool_b/4.png) |
...
| Bug ID   | Found by TC                            | Tiêu đề                                                                                                                                                                                                         | Severity | GitHub Issue | Screenshot |
| -------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------ | ---------- |
| BUG-C-01 | TC-ADMIN_ORDER-002, TC-ADMIN_ORDER-013 | Authorization bypass: `/api/admin/*` không kiểm tra `role=admin` → user thường đọc/sửa được đơn (server.js:510/525)                                                                                             | Critical | `<#issue>`   | `<ảnh>`    |
| BUG-C-02 | TC-ADMIN_ORDER-010                     | State machine bug: `canceled → delivered` được cho phép (server.js:550-551) — đơn đã hủy lại thành đã giao                                                                                                      | High     | `<#issue>`   | `<ảnh>`    |
| BUG-C-03 | TC-ADMIN_ORDER-016, TC-ADMIN_ORDER-017 | Thiếu input validation cho `status`: body thiếu field hoặc `status=""` không bị chặn (server.js:526) → lọt vào kiểm transition, trả thông điệp sai `"...to undefined"`/`"...to "` thay vì 400 "status bắt buộc" | Medium   | `<#issue>`   | `<ảnh>`    |
...
| Bug ID   | Found by TC                                              | Tiêu đề                                                                                                                                                                                                     | Severity | GitHub Issue | Screenshot |
| -------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------ | ---------- |
| BUG-D-01 | TC-MOB_REG-002..008, TC-MOB_REG-009..011, TC-MOB_REG-101 | Thiếu server-side validation: `POST /api/register` (server.js:20-30) không validate name/email/password — gửi rỗng, sai định dạng, thiếu field đều được INSERT thẳng vào DB                                 | High     | `<#issue>`   | `<ảnh>`    |
| BUG-D-02 | TC-MOB_REG-004                                           | Duplicate email: `database.js` không có UNIQUE constraint trên cột `email` + server không check trùng → đăng ký nhiều tài khoản cùng email thành công                                                       | High     | `<#issue>`   | `<ảnh>`    |
| BUG-D-03 | TC-MOB_REG-012                                           | Plaintext password storage: `server.js:23` INSERT password trực tiếp không qua hashing (bcrypt/argon2) → lộ mật khẩu nếu DB bị truy cập                                                                     | Critical | `<#issue>`   | `<ảnh>`    |
| BUG-D-04 | TC-MOB_REG-013                                           | Regex client không khớp whitelist SRS: `App.js:212` dùng `[^A-Za-z\d]` chấp nhận MỌI ký tự đặc biệt (`#`, `^`, dấu cách...), trong khi SRS FR-01 chỉ cho phép `@ $ ! % * ? &` → validation rộng hơn yêu cầu | Medium   | `<#issue>`   | `<ảnh>`    |
| BUG-D-05 | TC-MOB_REG-014                                           | Thiếu trường "Xác nhận mật khẩu": form Register mobile (`App.js`) không có confirm password — vi phạm SRS FR-01 (README.md:35) bắt buộc có trường xác nhận và từ chối khi không khớp                        | Medium   | `<#issue>`   | `<ảnh>`    |
...
| -------- | ------- | ---------------------------------- | ----------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------- | ---------------------------------------------------------------- |
| BUG-A-01 | A       | TC-PROFILE-012                     | Privilege escalation: user thường tự gán `role=admin` qua `PUT /api/users/me`                   | Critical | Confirmed (executed)                                  | [#6](https://github.com/trwng-thdat/software-testing/issues/6)   |
| BUG-A-02 | A       | TC-PROFILE-004                     | Stored XSS qua `name` + `dangerouslySetInnerHTML` (App.jsx:27)                                  | High     | Confirmed (executed)                                  | [#7](https://github.com/trwng-thdat/software-testing/issues/7)   |
| BUG-A-03 | A       | TC-PROFILE-002/003/005/007/008/010 | Thiếu validation server-side cho `name`/`phone`/`shipping_address`                              | High     | Confirmed (executed)                                  | [#8](https://github.com/trwng-thdat/software-testing/issues/8)   |
| BUG-A-04 | A       | TC-PROFILE-011                     | Stored XSS qua `shipping_address`                                                               | Medium   | Confirmed (executed)                                  | [#9](https://github.com/trwng-thdat/software-testing/issues/9)   |
| BUG-A-05 | A       | TC-PROFILE-006                     | Design bug regex phone: từ chối số VN hợp lệ bắt đầu bằng `0`                                   | Medium   | Confirmed (executed)                                  | [#10](https://github.com/trwng-thdat/software-testing/issues/10) |
| BUG-A-06 | A       | _(Mobile — execute ở Feature D)_   | Mobile field mismatch: App.js gửi `shippingAddress` (camelCase) ≠ server đọc `shipping_address` | Medium   | Open — xác nhận qua đọc code; cần execute trên mobile | [#11](https://github.com/trwng-thdat/software-testing/issues/11) |
| BUG-A-07 | A       | TC-PROFILE-015                     | Sensitive Data Exposure: `GET /api/users/me` `SELECT *` trả cả `password`/`reset_token`         | Critical | Confirmed (executed)                                  | [#12](https://github.com/trwng-thdat/software-testing/issues/12) |
| BUG-A-08 | A       | TC-PROFILE-016                     | Data loss: partial update ghi NULL đè dữ liệu cũ                                                | High     | Confirmed (executed)                                  | [#13](https://github.com/trwng-thdat/software-testing/issues/13) |
| BUG-B-01 | B       | TC-CHECKOUT-102                    | Off-by-one: apply-coupon dùng `>` thay vì `>=` (server.js:379)                                  | High     | Confirmed (executed)                                  | [#14](https://github.com/trwng-thdat/software-testing/issues/14) |
| BUG-B-02 | B       | TC-CHECKOUT-009, TC-CHECKOUT-103   | Sai công thức percent discount → discount âm, final > total (server.js:399)                     | Critical | Confirmed (executed)                                  | [#15](https://github.com/trwng-thdat/software-testing/issues/15) |
| BUG-B-03 | B       | TC-CHECKOUT-002…007                | Thiếu validation server-side cho checkout (`total_amount`, `shipping_address`)                  | High     | Confirmed (executed)                                  | [#16](https://github.com/trwng-thdat/software-testing/issues/16) |
| BUG-B-04 | B       | TC-CHECKOUT-015                    | Bypass `max_uses_per_user`: bỏ field `user_id` → bỏ qua kiểm tra số lần dùng                    | Critical | Confirmed (executed)                                  | [#17](https://github.com/trwng-thdat/software-testing/issues/17) |
| BUG-C-01 | C       | TC-ADMIN_ORDER-002, -013           | Authorization bypass: `/api/admin/*` không kiểm tra `role=admin`                                | Critical | Confirmed (executed)                                  | `<#issue>`                                                       |
| BUG-C-02 | C       | TC-ADMIN_ORDER-010                 | State machine bug: cho phép `canceled → delivered` (server.js:550-551)                          | High     | Confirmed (executed)                                  | `<#issue>`                                                       |
| BUG-C-03 | C       | TC-ADMIN_ORDER-016, -017           | Thiếu input validation cho `status` (thiếu field / rỗng) → thông điệp lỗi sai                   | Medium   | Confirmed (executed)                                  | `<#issue>`                                                       |
| BUG-D-01 | D       | TC-MOB_REG-002…011, -101           | Thiếu server-side validation `POST /api/register` (name/email/password)                         | High     | Confirmed (executed)                                  | `<#issue>`                                                       |
| BUG-D-02 | D       | TC-MOB_REG-004                     | Duplicate email: thiếu UNIQUE constraint + không check trùng                                    | High     | Confirmed (executed, count=2)                         | `<#issue>`                                                       |
| BUG-D-03 | D       | TC-MOB_REG-012                     | Plaintext password storage (server.js:23)                                                       | Critical | Confirmed (executed, đọc DB)                          | `<#issue>`                                                       |
| BUG-D-04 | D       | TC-MOB_REG-013                     | Regex client `[^A-Za-z\d]` rộng hơn whitelist SRS `{@$!%*?&}`                                   | Medium   | Confirmed (executed)                                  | `<#issue>`                                                       |
| BUG-D-05 | D       | TC-MOB_REG-014                     | Thiếu trường "Xác nhận mật khẩu" trên mobile — vi phạm FR-01                                    | Medium   | Confirmed (code review)                               | `<#issue>`                                                       |

> Mỗi bug phải có issue trên GitHub kèm screenshot; trong issue ghi rõ **Found by Test Case** và **Requirement liên quan**. Issue cho **Feature C & D** (`<#issue>`) sẽ được bổ sung sau.

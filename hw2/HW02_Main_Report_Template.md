...
| BUG-B-04 | TC-CHECKOUT-015                                                                                      | Bypass `max_uses_per_user`: `apply-coupon` không có `authenticateToken`; khi bỏ field `user_id`, nhánh `if (user_id)` (server.js:386) = false → bỏ qua toàn bộ kiểm tra số lần dùng → mã giới hạn lượt vẫn áp được vô hạn bởi caller ẩn danh | Critical | `<#issue>`   | `<ảnh>`    |
...
| BUG-C-03 | Thiếu input validation cho `status`: body thiếu field hoặc `status=""` không bị chặn (server.js:526 không validate enum/bắt buộc) → lọt vào kiểm transition và trả lỗi sai thông điệp `"...to undefined"`/`"...to "` thay vì 400 "status bắt buộc" | TC-ADMIN_ORDER-016, TC-ADMIN_ORDER-017 | Medium   | `<#issue>`   |
